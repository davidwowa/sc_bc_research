...
handle_data(Json, <<"keys">>) ->
  ...
  Doc = {[{keys, [public_key, base64:encode(PublicKey), private_key, base64:encode(PrivKeyOut), tt, TT]}]},
  Map = jsone:decode(Json),
  GUID = maps:get(<<"guid">>, Map),
  Ip = maps:get(<<"ip">>, Map),
  db_utils:save(GUID, base64:encode(PublicKey), Ip, TT),
  db_utils:save(base64:encode(PublicKey), base64:encode(PrivKeyOut), TT),
  jsone:encode(Doc);
...
handle_data(Json, <<"load">>) ->
  ...
  Doc = {[{load, [guid, Result#pseudonym.guid, public_key, Result#pseudonym.public_key, ip, Result#pseudonym.ip, tt, Result#pseudonym.timestamp]}]},
  jsone:encode(Doc);
handle_data(Json, _) ->
  %TODO
  lager:error(Json),
